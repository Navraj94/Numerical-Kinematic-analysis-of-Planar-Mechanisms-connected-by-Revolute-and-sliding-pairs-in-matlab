...
for i=1:length(tita)
    ...
    xc(1,i)=asin((h-l2*sin(tita(i)))/l3);
    xc(2,i)=l2*cos(tita(i))+sqrt((l3^2)-(h-l2*sin(tita(i)))^2);
    
    
    jac=sli_jaco(param,xc(:,i));
...
%     suptitle('Solution by Analytical Elimination of variables Method','bold','fontsize',16)
end
% plot(tita,vc(2,:),'-.','LineWidth',2)
    figure(1)
    ...
    hold on
    grid on
 hi = suptitle('Solution by Analytical Elimination of variables Method')
